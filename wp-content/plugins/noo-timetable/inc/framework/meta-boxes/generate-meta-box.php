...
if( !function_exists( 'noo_create_meta_box' ) ) {
	...
	function noo_create_meta_box( $post, $meta_box ) {
		...
		foreach ( $meta_box['fields'] as $field ) {
			...
			$id  = $field['id'];
			$meta = noo_timetable_get_post_meta( $post->ID, $id );
...
if( !function_exists( 'noo_render_metabox_fields' ) ) {
	...
	function noo_render_metabox_fields ( $post, $id, $type, $meta, $std, $field = null ) {
		switch( $type ) {
			...
			case 'gallery':
				...
				$btn_text = !empty( $meta ) ? esc_html__( 'Edit Gallery', 'noo-timetable' ) : esc_html__( 'Add Images', 'noo-timetable' );
				...
				echo '<input type="button" class="button" name="' . $id . '_button_clear" id="' . $id . '_clear" value="' . esc_html__( 'Clear Gallery', 'noo-timetable' ) . '" />';
	...
	?>
					...
					jQuery(document).ready(function($) {
						...
						$('#<?php echo esc_attr($id); ?>_upload').on('click', function(event) {
							...
							wp_media_frame.on('update', function(){
								...
								noo_upload_btn.attr('value', '<?php echo esc_html__( 'Edit Gallery', 'noo-timetable' ); ?>');
						...
						$('#<?php echo esc_attr($id); ?>_clear').on('click', function(event) {
							...
							noo_clear_btn.hide();
							$('#<?php echo esc_attr($id); ?>_upload').attr('value', '<?php echo esc_html__( 'Add Images', 'noo-timetable' ); ?>');
			...
			case 'media':
				...
				$val = $meta ? $meta : $std;
				$btn_text = !empty( $val ) ? esc_html__( 'Change File', 'noo-timetable' ) : esc_html__( 'Select File', 'noo-timetable' );
				echo '<input type="text" name="noo_meta_boxes[' . $id . ']" id="' . $id . '" value="' . ( $meta ? $meta : $std ) . '" style="margin-bottom:10px" />';
				echo '<input type="button" class="button button-primary" name="' . $id . '_button_upload" id="' . $id . '_upload" value="' . $btn_text . '" />';
				echo '<input type="button" class="button" name="' . $id . '_button_clear" id="' . $id . '_clear" value="' . esc_html__( 'Clear File', 'noo-timetable' ) . '" />';
				?>
				<script>
					jQuery(document).ready(function($) {
						...
						$('#<?php echo esc_attr($id); ?>_upload').on('click', function(event) {
							...
							var wp_media_frame = wp.media.frames.wp_media_frame = wp.media({
								title: "<?php echo esc_html__( 'Select or Upload your File', 'noo-timetable' ); ?>",
								button: {
									text: "<?php echo esc_html__( 'Select', 'noo-timetable' ); ?>"
								},
								<?php if($type == 'media'):?>
							...
							wp_media_frame.on('select', function(){
								...
								noo_upload_btn.attr('value', '<?php echo esc_html__( 'Change File', 'noo-timetable' ); ?>');
								$('#<?php echo esc_attr($id); ?>_clear').css('display', 'inline-block');
							});
						...
						$('#<?php echo esc_attr($id); ?>_clear').on('click', function(event) {
							var noo_clear_btn = $(this);
							noo_clear_btn.hide();
							$('#<?php echo esc_attr($id); ?>_upload').attr('value', '<?php echo esc_html__( 'Select File', 'noo-timetable' ); ?>');
							noo_clear_btn.siblings('#<?php echo esc_attr($id); ?>').val('');
						});
					});
				</script>
			...
			case 'image':
				...
				$btn_text = !empty( $image_id ) ? esc_html__( 'Change Image', 'noo-timetable' ) : esc_html__( 'Select Image', 'noo-timetable' );
				echo '<input type="hidden" name="noo_meta_boxes[' . $id . ']" id="' . $id . '" value="' . ( $meta ? $meta : $std ) . '" />';
				echo '<input type="button" class="button button-primary" name="' . $id . '_button_upload" id="' . $id . '_upload" value="' . $btn_text . '" />';
				echo '<input type="button" class="button" name="' . $id . '_button_clear" id="' . $id . '_clear" value="' . esc_html__( 'Clear Image', 'noo-timetable' ) . '" />';
				echo '<div class="noo-thumb-wrapper">' . $output . '</div>';
	?>
				<script>
					jQuery(document).ready(function($) {
						...
						$('#<?php echo esc_attr($id); ?>_upload').on('click', function(event) {
							...
							// I decided to create new frame every time to control the selected images
							var wp_media_frame = wp.media.frames.wp_media_frame = wp.media({
								title: "<?php echo esc_html__( 'Select or Upload your Image', 'noo-timetable' ); ?>",
								button: {
									text: "<?php echo esc_html__( 'Select', 'noo-timetable' ); ?>"
								},
								library: { type: 'image' },
							...
							wp_media_frame.on('select', function(){
								...
								noo_upload_btn.attr('value', '<?php echo esc_html__( 'Change Image', 'noo-timetable' ); ?>');
								$('#<?php echo esc_attr($id); ?>_clear').css('display', 'inline-block');
							});
						...
						$('#<?php echo esc_attr($id); ?>_clear').on('click', function(event) {
							var noo_clear_btn = $(this);
							noo_clear_btn.hide();
							$('#<?php echo esc_attr($id); ?>_upload').attr('value', '<?php echo esc_html__( 'Select Image', 'noo-timetable' ); ?>');
							noo_clear_btn.siblings('#<?php echo esc_attr($id); ?>').val('');
							noo_clear_btn.siblings('.noo-thumb-wrapper').html('');
						});
					});
			...
			/*case 'select_multiple_chosen':
				$meta = $meta ? $meta : $std;
				echo'<select id='.$id.' multiple="multiple" name="noo_meta_boxes[' . $id . '][]" class="js'.$id.'">';
				foreach ( $field['options'] as $option ) {
					$opt_value  = @$option['value'];
					$opt_label  = @$option['label'];
					echo '<option';
					echo ' value="'.$opt_value.'"';
					if (in_array($opt_value, $meta)) echo ' selected="selected"';
					echo '>' . $opt_label . '</option>';
				}
				echo '</select>'; */?><!--
                <script>
                    jQuery(document).ready(function($) {
                        $(".js<?php /*echo $id; */?>").chosen({
                            placeholder_text_multiple: '<?php /*echo esc_html__( "Select Organizers", "noo-timetable" ); */?>'
                        });
                        $(".js<?php /*echo $id; */?>").trigger('liszt:updated');
                    });
                </script>
				--><?php
/*				break;*/
	...
	?>
			...
			case 'page_layout':
				$post_layout = noo_timetable_get_option('noo_blog_post_layout', 'same_as_blog');
				$sidebar = '';
				if ($post_layout == 'same_as_blog') {
					$post_layout = noo_timetable_get_option( 'noo_blog_layout', 'sidebar' );
					$sidebar = noo_timetable_get_option('noo_blog_sidebar', 'sidebar-main');
				} else {
					$sidebar = noo_timetable_get_option('noo_blog_post_sidebar', 'sidebar-main');
				}

				$post_layout_text = '';
				switch( $post_layout ) {
					case 'fullwidth':
						$post_layout_text = esc_html__( 'Full-Width Page', 'noo-timetable' );
						break;
					case 'sidebar':
						$post_layout_text = esc_html__( 'Page With Right Sidebar', 'noo-timetable' );
						break;
					case 'left_sidebar':
						$post_layout_text = esc_html__( 'Page With Left Sidebar', 'noo-timetable' );
						break;
				}
				
				echo '<p>' . sprintf( noo_timetable_kses( __( 'Global setting for the Layout of Single Post page is: <strong>%s</strong>', 'noo-timetable') ), $post_layout_text ) . '</p>';
				if ( $post_layout != 'fullwidth' ) {
					$sidebar_text = noo_timetable_get_sidebar_name( $sidebar );
					echo '<p>' . sprintf( noo_timetable_kses( __( 'And the Sidebar is: <strong>%s</strong>', 'noo-timetable') ), $sidebar_text ) . '</p>';
				}
			...
			case 'menus':
				...
				echo'<select name="noo_meta_boxes[' . $id . ']" >';
				echo'	<option value="" '. selected( $meta, '', true ) . '>' . esc_html__( 'Don\'t Need Menu', 'noo-timetable') . '</option>';
			...
			case 'users':
				...
				echo'<select name="noo_meta_boxes[' . $id . ']" >';
				echo'	<option value="" '. selected( $meta, '', true ) . '>' . esc_html__( 'No User', 'noo-timetable') . '</option>';
			...
			case 'rev_slider':
				...
				echo '<select name="noo_meta_boxes[' . $id . ']">';
				echo '<option value="">' . esc_html__( ' - No Slider - ', 'noo-timetable') . '</option>';
...
if (!function_exists('noo_timetable_json_decode')) :
	function noo_timetable_json_decode( $json_str = '' ) {
		if( !is_string($json_str) ) return $json_str;
		$maybe_json = json_decode($json_str);
		if( !empty( $json_str ) && !is_array( $maybe_json ) ) {
			$maybe_json = @unserialize( $json_str );
		}
