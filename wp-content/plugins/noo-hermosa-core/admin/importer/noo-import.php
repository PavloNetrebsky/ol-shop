...
class Noo_Import_Demo extends WP_Importer{
	...
	public function __construct() {
		...
		// -- set ajax
			add_action( 'wp_ajax_hermosa_process_data', array( $this, 'hermosa_process_data' ) );

	}

	public function hermosa_process_data() {
		...
		// --- load file demo
			...
			echo esc_html__( 'Import Successfully!', 'noo-hermosa-core' );
	...
	function import_start( $file ) {
		if ( ! is_file($file) ) {
			echo '<p><strong>' . esc_html__( 'Sorry, there has been an error.', 'noo-hermosa-core' ) . '</strong><br />';
			echo esc_html__( 'The file does not exist, please try again.', 'noo-hermosa-core' ) . '</p>';
			$this->footer();
			die();
		}
	...
	function import_end() {
		...
		echo esc_html__( 'All done. Have fun!', 'noo-hermosa-core' );
	...
	function handle_upload() {
		...
		if ( isset( $file['error'] ) ) {
			...
			echo '<p><strong>' . esc_html__( 'Sorry, there has been an error.', 'noo-hermosa-core' ) . '</strong><br />';
			printf( esc_html__( 'The export file could not be found at <code>%s</code>. It is likely that this was caused by a permissions problem.', 'noo-hermosa-core' ), esc_html( $file['file'] ) );
			echo '</p>';
			return false;
		}
		...
		if ( $this->version > $this->max_NOO_version ) {
			...
			printf( esc_html__( 'This NOO file (version %s) may not be supported by this version of the importer. Please consider updating.', 'noo-hermosa-core' ), esc_html($import_data['version']) );
			echo '</strong></p></div>';
		}
	...
	function get_authors_from_import( $import_data ) {
		...
		// no author information, grab it from the posts
			...
			foreach ( $import_data['posts'] as $post ) {
				...
				if ( empty( $login ) ) {
					printf( esc_html__( 'Failed to import author %s. Their posts will be attributed to the current user.', 'noo-hermosa-core' ), esc_html( $post['post_author'] ) );
					echo '<br />';
					continue;
				}
	...
	function author_select( $n, $author ) {
		echo esc_html__( 'Import author:', 'noo-hermosa-core' );
		...
		if ( $create_users ) {
			if ( $this->version != '1.0' ) {
				echo esc_html__( 'or create new user with login name:', 'noo-hermosa-core' );
				$value = '';
			} else {
				echo esc_html__( 'as a new user:', 'noo-hermosa-core' );
		...
		if ( ! $create_users && $this->version == '1.0' )
			echo esc_html__( 'assign posts to an existing user:', 'noo-hermosa-core' );
		else
			echo esc_html__( 'or assign posts to an existing user:', 'noo-hermosa-core' );
		wp_dropdown_users( array( 'name' => "user_map[$n]", 'multi' => true, 'show_option_all' => esc_html__( '- Select -', 'noo-hermosa-core' ) ) );
			...
			if ( ! empty( $_POST['user_map'][$i] ) ) {
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG ) {
						printf( esc_html__( 'Failed to create new user for %s. Their posts will be attributed to the current user.', 'noo-hermosa-core' ), esc_html($this->authors[$old_login]['author_display_name']) );
	...
	function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( esc_html__( 'Failed to import category %s', 'noo-hermosa-core' ), esc_html($cat['category_nicename']) );
				if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG )
	...
	function process_tags() {
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG ) {
					printf( esc_html__( 'Failed to import post tag %s', 'noo-hermosa-core' ), esc_html($tag['tag_name']) );
					echo ': ' . $id->get_error_message();
					echo '<br />';
				}
				continue;
	...
	function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG ) {
					printf( esc_html__( 'Failed to import %s %s', 'noo-hermosa-core' ), esc_html($term['term_taxonomy']), esc_html($term['term_name']) );
					echo ': ' . $id->get_error_message();
					echo '<br />';
				}
				continue;
	...
	function process_posts() {
		...
		foreach ( $this->posts as $post ) {
			...
			if ( ! post_type_exists( $post['post_type'] ) ) {
				printf( esc_html__( 'Failed to import &#8220;%s&#8221;: Invalid post type %s', 'noo-hermosa-core' ),
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				// printf( esc_html__( '%s &#8220;%s&#8221; already exists.', 'noo-hermosa-core'), $post_type_object->labels->singular_name, esc_html($post['post_title']) );
				// echo '<br />';
				...
				if ( is_wp_error( $post_id ) ) {
					if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG ) {
						printf( esc_html__( 'Failed to import %s &#8220;%s&#8221;', 'noo-hermosa-core' ),
				...
				foreach ( $post['terms'] as $term ) {
					...
					if ( ! $term_id ) {
						...
						if ( ! is_wp_error( $t ) ) {
							...
							if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG ) {
								printf( esc_html__( 'Failed to import %s %s', 'noo-hermosa-core' ), esc_html($taxonomy), esc_html($term['name']) );
	...
	function process_menu_item( $item ) {
		...
		if ( ! $menu_slug ) {
			echo esc_html__( 'Menu item skipped due to missing menu slug', 'noo-hermosa-core' );
			echo '<br />';
			return;
		}
		...
		if ( ! $menu_id ) {
			printf( esc_html__( 'Menu item skipped due to invalid menu slug: %s', 'noo-hermosa-core' ), esc_html( $menu_slug ) );
			echo '<br />';
			return;
		} else {
	...
	function noo_update_nav_menu_item( $menu_id = 0, $menu_item_db_id = 0, $menu_item_data = array() ) {
	    ...
	    if ( ! empty( $menu_item_db_id ) && ! is_nav_menu_item( $menu_item_db_id ) )
	        return new WP_Error( 'update_nav_menu_item_failed', esc_html__( 'The given object ID is not that of a menu item.', 'noo-hermosa-core' ) );
	 
	    $menu = wp_get_nav_menu_object( $menu_id );
	 
	    if ( ! $menu && 0 !== $menu_id ) {
	        return new WP_Error( 'invalid_menu_id', esc_html__( 'Invalid menu ID.', 'noo-hermosa-core' ) );
	    }
	...
	function process_attachment( $post, $url ) {
		if ( ! $this->fetch_attachments )
			return new WP_Error( 'attachment_processing_error',
				esc_html__( 'Fetching attachments is not enabled', 'noo-hermosa-core' ) );
		...
		if ( $info = wp_check_filetype( $upload['file'] ) )
			...
			return new WP_Error( 'attachment_processing_error', esc_html__( 'Invalid file type', 'noo-hermosa-core' ) );
	...
	function fetch_remote_file( $url, $post ) {
		...
		if ( ! $headers ) {
			@unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', esc_html__( 'Remote server did not respond', 'noo-hermosa-core' ) );
		}

		// make sure the fetch was successful
		if ( $headers['response'] != '200' ) {
			@unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', sprintf( esc_html__( 'Remote server returned error response %1$d %2$s', 'noo-hermosa-core' ), esc_html($headers['response']), get_status_header_desc($headers['response']) ) );
		}
		...
		if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
			@unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', esc_html__( 'Remote file is incorrect size', 'noo-hermosa-core' ) );
		}

		if ( 0 == $filesize ) {
			@unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', esc_html__( 'Zero size file downloaded', 'noo-hermosa-core' ) );
		}

		$max_size = (int) $this->max_attachment_size();
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			@unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', sprintf( esc_html__( 'Remote file is too large, limit is %s', 'noo-hermosa-core' ), size_format($max_size) ) );
		}
	...
	function header() {
		echo '<div class="wrap">';
		echo '<h2>' . esc_html__( 'Import WordPress', 'noo-hermosa-core' ) . '</h2>';
		...
		if ( isset( $updates[$basename] ) ) {
			...
			echo '<div class="error"><p><strong>';
			printf( esc_html__( 'A new version of this importer is available. Please update to version %s to ensure compatibility with newer export files.', 'noo-hermosa-core' ), $update->update->new_version );
			echo '</strong></p></div>';
		}
	}
	...
	function greet() {
		...
		echo '<p>' . esc_html__( 'Howdy! Upload your WordPress eXtended RSS (NOO) file and we&#8217;ll import the posts, pages, comments, custom fields, categories, and tags into this site.', 'noo-hermosa-core' ) . '</p>';
		echo '<p>' . esc_html__( 'Choose a NOO (.xml) file to upload, then click Upload file and import.', 'noo-hermosa-core' ) . '</p>';
	...
	function _noo_process_import_file( $file ) {
		...
		// if ( ! file_exists( $file ) ) {
		// 	wp_die(
		// 		esc_html__( 'Import file could not be found. Please try again.', 'noo-hermosa-core' ),
		// 		'',
	...
	function _noo_import_data( $data ) {
		...
		if ( empty( $data ) || ! is_object( $data ) ) {
			wp_die(
				esc_html__( 'Import data could not be read. Please try a different file.', 'noo-hermosa-core' ),
				'',
				array( 'back_link' => true )
			);
		}
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
				...
				$sidebar_message = esc_html__( 'Sidebar does not exist in theme (using Inactive)', 'noo-hermosa-core' );
			}
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
					...
					$widget_message = esc_html__( 'Site does not support widget', 'noo-hermosa-core' ); // explain why widget not imported
				}
				...
				if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
					...
					foreach ( $single_widget_instances as $check_id => $check_widget ) {
						...
						if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
							...
							$widget_message = esc_html__( 'Widget already exists', 'noo-hermosa-core' ); // explain why widget not imported
				...
				if ( ! $fail ) {
					...
					if ( $sidebar_available ) {
						...
						$widget_message = esc_html__( 'Imported', 'noo-hermosa-core' );
					} else {
						$widget_message_type = 'warning';
						$widget_message = esc_html__( 'Imported to Inactive', 'noo-hermosa-core' );
					}
				...
				$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = ! empty( $widget['title'] ) ? $widget['title'] : esc_html__( 'No Title', 'noo-hermosa-core' ); // show "No Title" if widget instance is untitled
	...
	private function get_whitelist_options() {
		return apply_filters( 'options_import_whitelist', array(
			...
			'page_on_front',
			'show_on_front',
			'theme_mods_noo-hermosa',
			'theme_mods_noo-hermosa-child'
		) );
	}
