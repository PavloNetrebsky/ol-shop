...
function matthewruddy_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
		...
		if ( empty( $url ) )
			return new WP_Error( 'no_image_url', esc_html__( 'No image URL has been entered.','noo-hermosa' ), $url );
		...
		$file_path = $HTTP_SERVER_VARS ['DOCUMENT_ROOT'] . $file_path['path'];
		...
		$name = wp_basename( $file_path, ".$ext" );
	        ...
	        if ( 'bmp' == $ext ) {
			return new WP_Error( 'bmp_mime_type', esc_html__( 'Image is BMP. Please use either JPG or PNG.','noo-hermosa' ), $url );
		}
